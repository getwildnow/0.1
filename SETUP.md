...
# Fasten Health (Clinical Data)
FASTEN_PUBLIC_ID=your_fasten_public_id
FASTEN_API_KEY=your_fasten_private_key
...
- **Vital**: https://app.tryvital.io/ (Settings > API Keys)
- **Fasten**: https://connect.fastenhealth.com/ (Developer > API Clients)
...
## 4. Vital Setup (Health Data Integration)

1. **Create account** at https://app.tryvital.io/
2. **Get API credentials:**
   - Go to Settings > API Keys
   - Copy API Key
   - Add to `.env.local`
3. **Choose environment:**
   - Use `sandbox` for testing with simulated data
   - Use `production` for live health data
4. **Configure region:**
   - `us` for United States
   - `eu` for European Union

**Note:** In sandbox mode, you can test with Vital's test accounts without connecting real health devices.

---

## 5. Fasten Setup (Clinical Data/Medical Records)

1. **Create account** at https://connect.fastenhealth.com/
2. **Get API credentials:**
   - Go to Developer > API Clients
   - Copy Public ID (starts with `public_test_`)
   - Click eye icon to reveal Private Key
   - Add both to `.env.local`
3. **Choose environment:**
   - Use `sandbox` for testing with simulated medical records
   - Use `production` for live clinical data
4. **Optional - Configure webhook:**
   - URL: `https://your-domain.com/api/fasten/webhooks`
   - Events: `connection.created`, `connection.deleted`, `data.sync_completed`
   - Copy webhook secret and add to `.env.local`

**Note:** In sandbox mode, you can test with simulated medical records without connecting to real health systems.
...
## 7. Development Without Full Setup
...
- Health data integration (requires Vital)
- Clinical data/medical records (requires Fasten)
...
- Verify region matches your account (us vs eu)
- Ensure key starts with `sk_`

### "Fasten API error"
- Verify both Public ID and Private Key are correct
- Public ID starts with `public_test_` or `public_prod_`
- Private Key starts with `sk_test_` or `sk_prod_`
- Check environment (sandbox vs production)
- Ensure you've revealed the private key (click eye icon in dashboard)
